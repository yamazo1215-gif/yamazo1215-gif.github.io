...
const songsHtml=works.filter(w=>w.category!=='anime').map((w,i)=>`<article class="work-row"><span class="year">${esc(w.year||'—')}</span><div><h3>${w.source==='musicbrainz'?`<a href="${safe(w.url)}">${esc(w.title)}</a>`:esc(w.title)}</h3><p>${esc(w.detail)}</p></div><span class="role">${esc(w.role)}</span></article>`).join('');
