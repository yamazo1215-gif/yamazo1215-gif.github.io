...
const [anime,legacy,overrides,music]=await Promise.all(['anilist.json','legacy.json','overrides.json','musicbrainz.json'].map(read));
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const safe=s=>s&&new URL(s).protocol==='https:'?esc(s):'';
const works=[...anime,...legacy,...music].filter(w=>!w.mergedInto&&!overrides.excludedIds.includes(w.id)).map(w=>({...w,...overrides.records[w.id]})).sort((a,b)=>(b.year||0)-(a.year||0)||(b.startDate||'').localeCompare(a.startDate||''));
...
const songsHtml=works.filter(w=>w.category!=='anime').map((w,i)=>`<article class="work-row"><span class="year">${String(i+1).padStart(2,'0')}</span><div><h3>${w.source==='musicbrainz'?`<a href="${safe(w.url)}">${esc(w.title)}</a>`:esc(w.title)}</h3><p>${esc(w.detail)}</p></div><span class="role">${esc(w.role)}</span></article>`).join('');
